...
async function handleZoekwoning({ command, ack, respond, client }) {
  ...
  try {
    ...
    const fotocasaPromise = searchFotocasa(clientProfile.hard_filters)
      .then(r => { portalResults.fotocasa = r; return r; })
      .catch(err => { errors.push('Fotocasa'); console.error(`[${ts}] Fotocasa failed:`, err.message); return []; });

    const kyeroPromise = searchKyero(clientProfile.hard_filters)
      .then(r => { portalResults.kyero = r; return r; })
      .catch(err => { errors.push('Kyero'); console.error(`[${ts}] Kyero failed:`, err.message); return []; });

    // Progressive update: check every 10s which portals are done
    const allPromise = Promise.all([idealistaPromise, fotocasaPromise, kyeroPromise]);
    const progressInterval = setInterval(async () => {
      const parts = [];
      if (portalResults.idealista !== null) parts.push(`Idealista: ${portalResults.idealista.length}`);
      if (portalResults.fotocasa !== null) parts.push(`Fotocasa: ${portalResults.fotocasa.length}`);
      if (portalResults.kyero !== null) parts.push(`Kyero: ${portalResults.kyero.length}`);

      const done = parts.length;
      const waiting = 3 - done;
    ...
    const [idealistaListings, fotocasaListings, kyeroListings] = await allPromise;
    ...
    await updateStatus(client, channelId, threadTs,
      ...
      `Idealista: ${stats.idealistaCount} | Fotocasa: ${stats.fotocasaCount} | Kyero: ${stats.kyeroCount}\n\n` +
