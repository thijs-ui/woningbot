...
async function handleZoekwoning({ command, ack, respond, client }) {
  ...
  try {
    ...
    await updateStatus(client, channelId, threadTs,
      ...
      `:mag: *Stap 2/6* — Zoeken op Idealista + ThinkSpain...`);
    ...
    const thinkspainPromise = searchThinkSpain(hardFilters)
      .then(r => { portalResults.thinkspain = r; return r; })
      .catch(err => {
        errors.push('ThinkSpain');
        console.error(`[${ts}] ThinkSpain failed:`, err.message);
        return [];
      });
    ...
    const progressInterval = setInterval(async () => {
      ...
      if (done > 0 && waiting > 0) {
        await updateStatus(client, channelId, threadTs,
          `:mag: *Stap 2/6* — Portals doorzoeken...\n` +
          `:white_check_mark: ${parts.join(' | ')}\n` +
          `:hourglass_flowing_sand: Nog ${waiting} portal(s) bezig...`);
      }
    }, 10000);

    let [idealistaListings, thinkspainListings] = await allPromise;
    clearInterval(progressInterval);

    // Fix #9: Post-filter ThinkSpain by property type (not filtered server-side)
    if (hardFilters.property_type) {
      thinkspainListings = filterThinkSpainByType(thinkspainListings, hardFilters.property_type);
    }
