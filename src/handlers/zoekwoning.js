...
async function handleZoekwoning({ command, ack, respond, client }) {
  ...
  try {
    confirmMsg = await client.chat.postMessage({
      ...
      text: '🔍 Ik ga zoeken op Idealista. Even geduld...',
    });
  } catch (err) {
  ...
  try {
    ...
    // Step 4: Scrape portals in parallel
    console.log(`[${ts}] [ZoekWoning] Scraping portals...`);
    const errors = [];
    const [idealistaResult, fotocasaResult] = await Promise.allSettled([
    ...
    const allRaw = [...idealistaListings, ...fotocasaListings];

    if (allRaw.length === 0) {
      if (errors.length > 0) {
        await updateOrPost(client, channelId, threadTs,
          buildErrorBlocks(`Er ging iets mis bij het zoeken op ${errors.join(' en ')}. Probeer het over een paar minuten opnieuw.`));
      } else {
    ...
    if (selections.length === 0) {
      await updateOrPost(client, channelId, threadTs,
        buildNoResultsBlocks());
      // Store thread data anyway for refinement
      if (threadTs) {
        setThread(threadTs, {
    ...
    const blocks = buildResultBlocks(selections, allProperties, clientProfile, stats);
    ...
    if (errors.length > 0) {
      await client.chat.postMessage({
        channel: channelId,
        thread_ts: threadTs || undefined,
        text: `ℹ️ Let op: ${errors.join(' en ')} gaf een fout. Resultaten komen alleen van ${errors.includes('Idealista') ? 'Fotocasa' : 'Idealista'}.`,
      });
    }
