...
/**
 * Handle project info requests — search the web via Tavily (incl. Idealista pages).
 *
 * Uses 4 Tavily queries: NL, EN, ES + Idealista-specific.
 * The Idealista query uses include_domains to find the actual project page,
 * rather than scraping random new-build listings and fuzzy-matching.
 */
async function handleProjectInfo(client, channelId, threadTs, query, projectName, projectLocation, ts) {
  ...
  try {
    ...
    console.log(`[${ts}] [Nieuwbouw] Project info delivered for "${projectName}" (${result.source_count} sources, ${result.idealista_count} from Idealista)`);
