...
const { classifyIntent, getProjectInfo, searchProjectInfo, summarizeProjectInfo } = require('../services/project-info');
...
async function handleNieuwbouw({ command, ack, client }) {
  ...
  try {
    ...
    const classifyResult = await classifyIntent(query);
    const { intent, project_name: detectedProject } = classifyResult;
    console.log(`[${ts}] [Nieuwbouw] Intent: ${intent}, detected project: ${detectedProject || 'none'}`);

    // ─── Branch: PROJECT INFO ────────────────────────────────────────
    if (intent === 'project_info' && detectedProject) {
      const detectedLocation = classifyResult.location || null;
      await handleProjectInfo(client, channelId, threadTs, query, detectedProject, detectedLocation, ts);
      return;
    }
...
/**
 * Handle project info requests — search the web AND Idealista in parallel.
 *
 * @param {string} projectLocation - City inferred from the project name (e.g. "Marbella")
 */
async function handleProjectInfo(client, channelId, threadTs, query, projectName, projectLocation, ts) {
  console.log(`[${ts}] [Nieuwbouw] PROJECT INFO flow for "${projectName}" (location: ${projectLocation || 'unknown'})`);

  await updateStatus(client, channelId, threadTs,
    `:building_construction: *NieuwbouwBot — Project Info*\n\n` +
    `:dart: Project: *${projectName}*\n` +
    (projectLocation ? `:round_pushpin: Locatie: ${projectLocation}\n` : '') +
    `:globe_with_meridians: Internet doorzoeken + beschikbare woningen ophalen...`);

  try {
    // ─── Run Tavily + Idealista in parallel ──────────────────────────
    const idealistaLocations = projectLocation ? [projectLocation] : [];
    const idealistaFilters = {
      locations: idealistaLocations,
      is_new_build: true,
      // Search broadly — no price/bedroom filters
      price_min: null,
      price_max: null,
      bedrooms_min: null,
      operation: 'sale',
    ...
    const [tavilyResult, idealistaResult] = await Promise.allSettled([
      searchProjectInfo(projectName),
      idealistaLocations.length > 0 ? searchIdealista(idealistaFilters) : Promise.resolve([]),
    ]);

    // Extract Tavily raw search results
    const tavilyData = tavilyResult.status === 'fulfilled'
      ? tavilyResult.value
      : { raw_context: '', results: [], source_count: 0 };

    if (tavilyResult.status === 'rejected') {
      console.error(`[${ts}] [Nieuwbouw] Tavily failed:`, tavilyResult.reason?.message);
    }

    // Extract and filter Idealista results by project name
    const allIdealistaListings = idealistaResult.status === 'fulfilled'
      ? idealistaResult.value
      : [];

    if (idealistaResult.status === 'rejected') {
      console.error(`[${ts}] [Nieuwbouw] Idealista failed:`, idealistaResult.reason?.message);
    }

    // Filter listings that match the project name
    const projectNameLower = projectName.toLowerCase();
    const projectWords = projectNameLower.split(/\s+/).filter(w => w.length > 2);

    const matchedListings = allIdealistaListings.filter(l => {
      const searchText = `${l.title || ''} ${l.description || ''} ${l.address || ''}`.toLowerCase();
      // Match if the full project name appears, or if most significant words match
      if (searchText.includes(projectNameLower)) return true;
      const matchingWords = projectWords.filter(w => searchText.includes(w));
      return matchingWords.length >= Math.ceil(projectWords.length * 0.6);
    });

    console.log(`[${ts}] [Nieuwbouw] Idealista: ${allIdealistaListings.length} total listings, ${matchedListings.length} matched "${projectName}"`);

    // ─── Summarize with Claude (one call, all data) ─────────────────
    const sources = tavilyData.results?.map(r => ({ title: r.title, url: r.url })) || [];
    const sourceCount = tavilyData.source_count || 0;

    const summary = await summarizeProjectInfo(
      projectName,
      tavilyData.raw_context || '',
      sourceCount,
      query,
      matchedListings
    );
    ...
    const listingsNote = matchedListings.length > 0
      ? ` + ${matchedListings.length} beschikbare woning(en) op Idealista`
      : '';

    const finalText = `:building_construction: *NieuwbouwBot — ${projectName}*\n` +
      `_${sourceCount} webbronnen${listingsNote}_\n\n` +
      `${summary}\n\n` +
    ...
    // Store thread data for follow-up questions (include listings for context)
    if (threadTs) {
      setThread(threadTs, {
        ...
        matched_listings: matchedListings.map(l => ({
          id: l.id, title: l.title, price: l.price, property_type: l.property_type,
          location: l.location, bedrooms: l.bedrooms, size_m2: l.size_m2,
          features: l.features, url: l.url, source: l.source,
        })),
    ...
    console.log(`[${ts}] [Nieuwbouw] Project info delivered for "${projectName}" (${sourceCount} web sources, ${matchedListings.length} listings)`);
