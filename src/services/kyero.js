 ...
 * Derived from Kyero.com URL patterns: /en/{city}-property-for-sale-0l{locationId}
 */
const KYERO_LOCATION_MAP = {
  // Costa del Sol
  'estepona': 31466,
  'marbella': 31476,
  'san pedro de alcantara': 31476,
  'malaga': 31473,
  'málaga': 31473,
  'fuengirola': 31468,
  'mijas': 31479,
  ...
  'torrevieja': 31326,
  'orihuela': 31316,
  'orihuela costa': 31316,
  'guardamar del segura': 31305,
  'rojales': 31320,
  'pilar de la horadada': 31318,
  'santa pola': 31323,
  'alicante': 31284,
  'elche': 31299,
...
/**
 * Map property type to Kyero URL suffix.
 * g1 = apartments, g2 = villas/houses, g4 = country houses
 */
...
/**
 * Build a Kyero search URL from hard filters.
 */
function buildKyeroUrl(hardFilters) {
  const city = (hardFilters.location || '').toLowerCase().trim();
  ...
  if (!locationId) {
    console.warn(`[Kyero] Location "${city}" not in mapping. Skipping Kyero search.`);
    return null;
  }
  ...
  // Build property type label for URL
...
async function searchKyero(hardFilters) {
  const searchUrl = buildKyeroUrl(hardFilters);

  if (!searchUrl) {
    console.log('[Kyero] No valid URL could be built. Returning empty results.');
    return [];
  }
  ...
  console.log('[Kyero] Starting Apify run with URL:', searchUrl);
  ...
  console.log(`[Kyero] Retrieved ${items.length} items`);

  // Filter out non-property items (agent listings etc.)
...
function normalizeKyero(item) {
  // Use the lower-res photos for thumbnails, full-res images for analysis
  ...
  return {
    ...
    thumbnail: thumbnail,
    features: features,
    property_type: item.property_type?.key || null,
