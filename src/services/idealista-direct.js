...
function mapApifyItem(item, sourceCity) {
  try {
    ...
    // Safely extract labels (Apify may return objects, strings, or mixed)
    const rawLabels = Array.isArray(item.labels) ? item.labels : [];
    const labelStrings = rawLabels.map(l => {
      if (typeof l === 'string') return l;
      if (l && typeof l === 'object') return l.text || l.name || l.label || JSON.stringify(l);
      return String(l || '');
    });

    // New build detection
    const isNewBuild = !!(item.newDevelopment || item.isNewDevelopment ||
      labelStrings.some(l => l.toLowerCase().includes('obra nueva') || l.toLowerCase().includes('new')));

    // Tags
    const tags = [];
    if (labelStrings.length) tags.push(...labelStrings);
