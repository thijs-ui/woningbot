 ...
 *
 * dz_omar actor returns a FLAT format:
 *   { thumbnail, price, address, municipality, province, description,
 *     rooms, bathrooms, size, url, sourceUrl, scrapedAt }
 */
function mapApifyItem(item, sourceCity) {
  try {
    // Skip items without price or URL
    if (!item.price && !item.url) return null;

    const price = typeof item.price === 'number' ? item.price : parseInt(String(item.price || '0').replace(/[^0-9]/g, ''), 10) || null;
    ...
    const city = item.municipality || sourceCity || '';
    const province = item.province || '';
    const address = item.address || '';

    // Property characteristics — flat fields
    ...
    // Features — extract from description since flat format has no feature flags
    const features = extractFeaturesFromDescription(description);

    // Property type — guess from description and URL
    const propertyType = guessPropertyType(description, url, item.sourceUrl || '');

    // New build detection
    const descLower = description.toLowerCase();
    const isNewBuild = descLower.includes('obra nueva') || descLower.includes('new development') ||
      descLower.includes('new build') || (item.sourceUrl || '').includes('obra-nueva');

    // Title
    const title = `${propertyType || 'Property'} in ${city}${address ? ` — ${address}` : ''}`;
...
/**
 * Guess property type from description and URL.
 */
function guessPropertyType(description, url, sourceUrl) {
  const desc = description.toLowerCase();
  const src = (sourceUrl || '').toLowerCase();

  // Check sourceUrl first (most reliable — it's the search filter)
  if (src.includes('chalets-independientes')) return 'chalet';
  if (src.includes('con-pisos')) return 'flat';
  if (src.includes('con-aticos')) return 'penthouse';
  if (src.includes('con-adosados')) return 'townhouse';
  if (src.includes('con-duplex')) return 'duplex';
  if (src.includes('con-rusticas')) return 'countryHouse';
  if (src.includes('con-estudios')) return 'studio';
  if (src.includes('con-chalets')) return 'chalet';

  // Fallback: guess from description
  if (desc.includes('villa ') || desc.includes('detached')) return 'chalet';
  if (desc.includes('penthouse') || desc.includes('ático')) return 'penthouse';
  if (desc.includes('townhouse') || desc.includes('adosado') || desc.includes('semi-detached')) return 'townhouse';
  if (desc.includes('duplex') || desc.includes('dúplex')) return 'duplex';
  if (desc.includes('bungalow')) return 'bungalow';
  if (desc.includes('finca') || desc.includes('country house') || desc.includes('cortijo')) return 'countryHouse';
  if (desc.includes('apartment') || desc.includes('apartamento') || desc.includes('flat') || desc.includes('piso')) return 'flat';

  return null;
}

/**
 * Extract features from description text (flat Apify format has no feature flags).
 */
function extractFeaturesFromDescription(description) {
  const features = [];
  const desc = (description || '').toLowerCase();

  if (desc.includes('pool') || desc.includes('piscina')) features.push('pool');
  if (desc.includes('garden') || desc.includes('jardín') || desc.includes('jardin')) features.push('garden');
  if (desc.includes('terrace') || desc.includes('terraza')) features.push('terrace');
  if (desc.includes('garage') || desc.includes('garaje') || desc.includes('parking')) features.push('garage');
  if (desc.includes('air condition') || desc.includes('aire acondicionado') || desc.includes('a/c')) features.push('air_conditioning');
  if (desc.includes('elevator') || desc.includes('lift') || desc.includes('ascensor')) features.push('elevator');
  if (desc.includes('storage') || desc.includes('trastero')) features.push('storage');
  if (desc.includes('sea view') || desc.includes('vista al mar') || desc.includes('vistas al mar') || desc.includes('ocean view')) features.push('sea_view');
  if (desc.includes('mountain view') || desc.includes('vista a la montaña')) features.push('mountain_view');
  if (desc.includes('security') || desc.includes('gated') || desc.includes('vigilancia') || desc.includes('secure')) features.push('security');
