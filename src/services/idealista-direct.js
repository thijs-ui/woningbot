...
const PROPERTY_TYPE_SLUG = {
  villa:         'chalets-independientes',  // ONLY detached villas (not con-chalets which includes semi-detached)
  apartment:     'pisos',
  penthouse:     'aticos',
  duplex:        'duplex',
  townhouse:     'adosados',
  finca:         'rusticas',
  country_house: 'rusticas',
  studio:        'estudios',
  bungalow:      'chalets',
...
/**
 * Build Idealista search URL using Spanish path-based filters.
 *
 * Uses the same URL format as the Idealista website itself:
 *   /en/venta-viviendas/{city-slug}/con-{type},{price},{bedrooms},{features}/
 *
 * This format is more reliable than query parameters and matches
 * what a real user would see in their browser.
 *
 * Example output:
 *   /en/venta-viviendas/marbella-malaga/con-chalets-independientes,precio-desde_800000,precio-hasta_1000000,de-tres-dormitorios,de-cuatro-cinco-habitaciones-o-mas,piscina/
 */
function buildSearchUrl(citySlug, hardFilters, pageNum = 1, isNewBuild = false) {
  ...
  // Build filter segments (comma-separated in the "con-" path)
  const segments = [];

  // Property type
  if (hardFilters.property_type && PROPERTY_TYPE_SLUG[hardFilters.property_type]) {
    segments.push(PROPERTY_TYPE_SLUG[hardFilters.property_type]);
  }

  // Price range
  if (hardFilters.price_min) segments.push(`precio-desde_${hardFilters.price_min}`);
  if (hardFilters.price_max) segments.push(`precio-hasta_${hardFilters.price_max}`);

  // Size range
  if (hardFilters.size_min_m2) segments.push(`metros-cuadrados-mas-de_${hardFilters.size_min_m2}`);
  if (hardFilters.size_max_m2) segments.push(`metros-cuadrados-menos-de_${hardFilters.size_max_m2}`);

  // Bedrooms — Idealista uses specific path segments for bedroom counts
  if (hardFilters.bedrooms_min) {
    const bedroomSegments = buildBedroomSegments(hardFilters.bedrooms_min, hardFilters.bedrooms_max);
    segments.push(...bedroomSegments);
  }

  // Features
  if (hardFilters.features && hardFilters.features.length) {
    for (const feat of hardFilters.features) {
      const seg = FEATURE_SLUG[feat];
      if (seg) segments.push(seg);
    }
  }

  // Build URL
  let url = `https://www.idealista.com/en/${basePath}/${citySlug}/`;

  // Add filter path if any segments
  if (segments.length > 0) {
    url += `con-${segments.join(',')}/`;
  }
...
/**
 * Build bedroom filter segments.
 * Idealista uses: de-un-dormitorio, de-dos-dormitorios, de-tres-dormitorios,
 * de-cuatro-cinco-habitaciones-o-mas
 *
 * For bedrooms_min=3: we need "de-tres-dormitorios,de-cuatro-cinco-habitaciones-o-mas"
 * This tells Idealista: show 3-bedroom AND 4+-bedroom properties.
 */
function buildBedroomSegments(min, max) {
  const BEDROOM_SLUGS = {
    1: 'de-un-dormitorio',
  ...
  const segments = [];
  const effectiveMax = max || 4; // 4 means "4 or more"

  for (let i = Math.max(1, min); i <= Math.min(4, effectiveMax); i++) {
    if (BEDROOM_SLUGS[i]) segments.push(BEDROOM_SLUGS[i]);
  }
...
function fetchPage(url, retryCount = 0) {
  return new Promise((resolve, reject) => {
    ...
    if (USE_PROXY) {
      // Route through ScraperAPI with render=true to bypass DataDome anti-bot
      const encodedUrl = encodeURIComponent(url);
      requestUrl = `http://api.scraperapi.com?api_key=${SCRAPER_API_KEY}&url=${encodedUrl}&country_code=es&render=true&premium=true`;
      options = {
        method: 'GET',
        headers: {
...
function parseTotalResults(html) {
  ...
  if (numMatch) {
    ...
    if (!isNaN(total) && total > 0) {
      const maxPages = Math.min(Math.ceil(total / 30), 60);
  ...
  let maxPage = 0;
  $('a.pagination-link, .pagination a, .next-prev-links a, [class*="pagination"] a').each((i, el) => {
  ...
  // CRITICAL FALLBACK: if we got listings but can't parse pagination,
  // always scrape MAX_PAGES_PER_LOCATION. Better to get 404s on empty
  // pages than to miss 80% of results.
  console.log(`[Idealista-Direct] Could not parse pagination — defaulting to ${MAX_PAGES_PER_LOCATION} pages`);
  return { total: null, maxPages: MAX_PAGES_PER_LOCATION };
}
...
async function scrapeCity(city, hardFilters, isNewBuild) {
  ...
  if (page1Listings.length === 0) {
    // Detailed diagnostics for empty results
    const bodyLen = page1Result.body.length;
    const hasArticles = page1Result.body.includes('article');
    const hasDataDome = page1Result.body.includes('datadome') || page1Result.body.includes('DataDome');
    const hasCaptcha = page1Result.body.includes('captcha') || page1Result.body.includes('challenge');
    const hasNoResults = page1Result.body.includes('no-results') || page1Result.body.includes('sin resultados');
    const hasH1 = page1Result.body.includes('h1-container');

    console.warn(`[Idealista-Direct] ${label}: 0 listings parsed. Diagnostics:`);
    console.warn(`  Body length: ${bodyLen} bytes`);
    console.warn(`  Has <article>: ${hasArticles}`);
    console.warn(`  Has DataDome: ${hasDataDome}`);
    console.warn(`  Has CAPTCHA: ${hasCaptcha}`);
    console.warn(`  Has no-results: ${hasNoResults}`);
    console.warn(`  Has h1-container: ${hasH1}`);
    console.warn(`  First 500 chars: ${page1Result.body.substring(0, 500).replace(/\n/g, ' ')}`);

    if (hasDataDome || hasCaptcha) {
      console.error(`[Idealista-Direct] BLOCKED by DataDome/CAPTCHA. ScraperAPI may need render=true or premium proxies.`);
    } else if (bodyLen < 5000) {
      console.warn(`[Idealista-Direct] ${label}: Very short response (${bodyLen} bytes) — possible block or redirect`);
    } else if (!hasArticles) {
      console.warn(`[Idealista-Direct] ${label}: HTML received but no <article> elements — page structure may have changed`);
    }
