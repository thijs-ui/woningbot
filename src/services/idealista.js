...
function buildApifyInput(hardFilters) {
  const locationId = resolveLocationId(hardFilters);

  const input = {
    country: 'es',
    operation: hardFilters.operation || 'sale',
    endpoint: hardFilters.property_type === 'plot' ? 'listlands' : 'listhomes',
    numPages: 2,
    language: 'en',
...
async function searchIdealista(hardFilters) {
  const apifyInput = buildApifyInput(hardFilters);
  console.log('[Idealista] Starting Apify run with input:', JSON.stringify(apifyInput, null, 2));
...
module.exports = { searchIdealista, buildApifyInput, LOCATION_MAP };
