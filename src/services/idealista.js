...
const LOCATION_MAP = {
  ...
  'san pedro de alcantara': '0-EU-ES-29-03-003-069', // Part of Marbella
  ...
  'sotogrande':     '0-EU-ES-11-03-005-033', // San Roque
  ...
  'moraira':        '0-EU-ES-03-01-005-128', // Teulada
 ...
 */
function resolveLocationId(filters) {
  const city = (filters.location || '').toLowerCase().trim();
  if (city && LOCATION_MAP[city]) return LOCATION_MAP[city];

  // Try partial match
  for (const [key, id] of Object.entries(LOCATION_MAP)) {
    if (city.includes(key) || key.includes(city)) return id;
  }

  console.warn(`[Idealista] Location "${city}" not in mapping. Falling back to province or no location filter.`);
  return null;
}
...
function buildApifyInput(hardFilters, forceEndpoint = null) {
  const locationId = resolveLocationId(hardFilters);
  ...
  const input = {
    ...
    numPages: 2,
    language: 'en',
    locationName: hardFilters.location || 'España',
  };
...
/**
 * Search Idealista via Apify.
 * If is_new_build is true, runs two parallel searches: resale + new homes.
...
async function searchIdealista(hardFilters) {
  ...
  if (isNewBuild) {
    console.log('[Idealista] New build requested — running parallel resale + obra nueva search');
    const [resaleResult, newBuildResult] = await Promise.allSettled([
      runSingleSearch(hardFilters, 'listhomes'),
      runSingleSearch(hardFilters, 'listnewhomes'),
    ]);

    const resale = resaleResult.status === 'fulfilled' ? resaleResult.value : [];
    const newBuild = newBuildResult.status === 'fulfilled' ? newBuildResult.value : [];

    if (resaleResult.status === 'rejected') console.error('[Idealista] Resale search failed:', resaleResult.reason?.message);
    if (newBuildResult.status === 'rejected') console.error('[Idealista] New build search failed:', newBuildResult.reason?.message);

    // Mark new build properties
    newBuild.forEach(p => { p.is_new_build = true; });

    console.log(`[Idealista] Resale: ${resale.length}, New build: ${newBuild.length}`);
    return [...newBuild, ...resale]; // Prioritize new build results
  }

  return runSingleSearch(hardFilters);
}

async function runSingleSearch(hardFilters, forceEndpoint = null) {
  const apifyInput = buildApifyInput(hardFilters, forceEndpoint);
  console.log(`[Idealista] Starting Apify run (endpoint: ${apifyInput.endpoint}) with input:`, JSON.stringify(apifyInput, null, 2));
  ...
  if (runResponse.statusCode >= 400) {
    console.error('[Idealista] Apify run failed:', JSON.stringify(runResponse.data));
    throw new Error(`Apify run failed with status ${runResponse.statusCode}`);
  }

  const run = runResponse.data.data;
  console.log(`[Idealista] Run status: ${run.status}, dataset: ${run.defaultDatasetId}`);

  if (run.status === 'RUNNING') {
    console.log('[Idealista] Still running, waiting 30s more...');
  ...
  // Filter out error/status items and only keep actual property listings
  const propertyItems = items.filter((item) =>
    item.status === 'success' && item.propertyCode
  );

  console.log(`[Idealista] Retrieved ${items.length} items, ${propertyItems.length} actual properties`);

  // Limit to 30 properties
  return propertyItems.slice(0, 30).map(normalizeIdealista);
}
