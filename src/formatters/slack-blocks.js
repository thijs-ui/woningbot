...
function getSourceLabel(source) {
  switch (source) {
    case 'fotocasa': return 'Fotocasa';
    case 'kyero': return 'Kyero';
...
function buildResultBlocks(selections, allProperties, clientProfile, stats, photoAssessments) {
  ...
  if (stats.kyeroCount > 0) sourcesParts.push(`Kyero (${stats.kyeroCount})`);
...
function buildConfirmationBlocks() {
  return [{ type: 'section', text: { type: 'mrkdwn', text: '🔍 Ik ga zoeken op Idealista, Fotocasa en Kyero. Even geduld...' } }];
}
